...
A root Codex session uses Codex Harness Mode. Sol is the default coordinator. If
another model is active, it is the root coordinator and must identify itself
accurately. A trusted `DELEGATED_TASK` header activates the worker contract
instead. A delegated worker has no coordinator, git, integration, or release
authority.
